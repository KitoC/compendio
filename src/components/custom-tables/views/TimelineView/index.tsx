...
import { CustomTableViewProps, TimeScale } from "../types";
...
const TimelineView = ({
  ...
  // const { startTime, endTime } = getTimeRange(timeScale);
  ...
  }, [records, startDateField, endDateField, primaryField, onEdit]);
  ...
  const handleItemResize = async (
    ...
    if (!updateRecord || !endDateField || endDateField === startDateField)
      return;
  ...
  if (records.length === 0 && !isLoadingData) {
    return (

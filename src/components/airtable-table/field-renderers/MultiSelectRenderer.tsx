  ...
  return (
    <div className="flex flex-wrap gap-1 max-w-[200px]">
      {value.map((id: string, index: number) => {
        ...
        if (!choice) {
          return (
            <span key={`${id}-${index}`} className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
              {id}
            </span>
          );
        }
        ...
        return (
          <span key={`${id}-${index}`} className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${colorClass}`}>
            {choice.name}
          </span>
        );
      })}

...
(function () {
  ...
  // Create a style target element for styled-components
  const styledComponentsTarget = document.createElement("div");
  styledComponentsTarget.id = "styled-components-target";
  shadowRoot.appendChild(styledComponentsTarget);
  ...
  fetch(cssUrl)
    ...
    .then((cssText) => {
      ...
      // Render React component into Shadow DOM with StyleSheetManager
      const root = createRoot(shadowContainer);
      root.render(
        React.createElement(
          StyleSheetManager,
          { target: styledComponentsTarget },
    ...
    .catch((error) => {
      ...
      // Render even if styles fail to load, still with StyleSheetManager
      const root = createRoot(shadowContainer);
      root.render(
        React.createElement(
          StyleSheetManager,

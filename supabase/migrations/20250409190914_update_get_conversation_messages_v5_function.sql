...
CREATE OR REPLACE FUNCTION public.get_conversation_messages_v5(
...
BEGIN
  ...
  IF filter ? 'tags' AND jsonb_typeof(filter->'tags') = 'array' THEN
    dynamic_filter := dynamic_filter || ' AND (m.metadata->''tags'') ?| array[';
  ...
  IF jsonb_typeof(filter) = 'object' THEN
    FOR key IN SELECT * FROM jsonb_object_keys(filter) LOOP
      IF position('.' IN key) > 0 AND key NOT IN ('metadata.status', 'tags') THEN
  ...
  sql := format(
    ...
    shared_message_target_cols(_decryption_key),
    dynamic_filter,
    sort_target_col
  );
